...
// CONFIGURAÇÕES (MOVER PARA SECRETS DO CLOUDFLARE)
const ADMIN_PASSWORD = 'aDMIN173@'; // MUDE ISSO!
const PRODUCT_PRICE = 47.00;
const DEFAULT_COMMISSION_PERCENT = 30; // 30% padrão para afiliados

// VOCÊ PRECISA CONFIGURAR ESTAS VARIÁVEIS NO CLOUDFLARE:
// - ASAAS_API_KEY: Sua chave da API Asaas
// - ASAAS_WALLET_ID: ID da sua carteira Asaas (opcional)
  ...
  async fetch(request, env) {
    ...
    if (!env.TIKTOK_ORDERS && url.pathname.startsWith('/api/')) {
      return new Response(JSON.stringify({
        error: 'Configure o KV Namespace TIKTOK_ORDERS no Worker'
      }), {
        status: 500,
    ...
    // Checkout (com suporte a afiliado via ?ref=CODIGO)
    ...
    // Painel do Afiliado
    if (url.pathname === '/affiliate') {
      return new Response(AFFILIATE_PANEL_HTML, {
        headers: { 'Content-Type': 'text/html;charset=UTF-8' }
      });
    }

    // Registro de Afiliado
    if (url.pathname === '/affiliate/register') {
      return new Response(AFFILIATE_REGISTER_HTML, {
        headers: { 'Content-Type': 'text/html;charset=UTF-8' }
      });
    }
    ...
    // Criar Pedido (com integração Asaas)
    ...
    if (url.pathname === '/api/affiliate/stats' && request.method === 'POST') {
      return handleAffiliateStats(request, env, corsHeaders);
    }

    // Vendas do afiliado
    if (url.pathname === '/api/affiliate/sales' && request.method === 'POST') {
      return handleAffiliateSales(request, env, corsHeaders);
    }
    ...
    if (url.pathname === '/api/admin/update-commission' && request.method === 'POST') {
      return handleUpdateCommission(request, env, corsHeaders);
    }

    // Aprovar/bloquear afiliado
    if (url.pathname === '/api/admin/toggle-affiliate' && request.method === 'POST') {
      return handleToggleAffiliate(request, env, corsHeaders);
    }

    // Marcar pedido como pago manualmente
    if (url.pathname === '/api/admin/mark-paid' && request.method === 'POST') {
      return handleMarkPaid(request, env, corsHeaders);
    }

    // Deletar pedido
    if (url.pathname === '/api/admin/delete-order' && request.method === 'POST') {
      return handleDeleteOrder(request, env, corsHeaders);
    }
...
async function handleCreateOrder(request, env, corsHeaders) {
  try {
    ...
    if (data.affiliateCode) {
      ...
      if (affiliateData) {
        ...
        if (affiliate.status === 'active') {
          commissionPercent = affiliate.commission || DEFAULT_COMMISSION_PERCENT;
        }
      }
    }
    ...
    const asaasResponse = await createAsaasCharge(env, {
      ...
      billingType: 'PIX',
      value: PRODUCT_PRICE,
      dueDate: getDateInDays(1), // Vence em 1 dia
      description: `TikTok Shop UK Masterclass - ${txid}`,
      externalReference: orderId
    });

    if (!asaasResponse.success) {
      return jsonResponse({ error: 'Erro ao gerar pagamento: ' + asaasResponse.error }, 500, corsHeaders);
    }

    const asaasData = asaasResponse.data;

    // Calcular comissão
    const commissionAmount = affiliate ? (PRODUCT_PRICE * commissionPercent / 100) : 0;
    ...
    // Se tem afiliado, adicionar à lista de vendas dele
    if (affiliate) {
      ...
      for (const orderId of ordersIds) {
        ...
        if (orderData) {
          ...
          if (order.asaasId === asaasId && order.status === 'pending') {
            ...
            // TODO: Aqui você pode enviar email/WhatsApp automático
...
async function handleAffiliateRegister(request, env, corsHeaders) {
  try {
    ...
    if (existing) {
      return jsonResponse({ error: 'Código de afiliado já existe. Tente outro nome.' }, 400, corsHeaders);
    }
    ...
    const affiliate = {
      ...
      email: data.email,
      whatsapp: data.whatsapp,
      password: await hashPassword(data.password), // Em produção, use bcrypt
      commission: DEFAULT_COMMISSION_PERCENT,
...
async function handleAffiliateLogin(request, env, corsHeaders) {
  try {
    ...
    if (!code || !password) {
      return jsonResponse({ error: 'Código e senha obrigatórios' }, 400, corsHeaders);
    }
    ...
    const validPassword = await verifyPassword(password, affiliate.password);
    if (!validPassword) {
      return jsonResponse({ error: 'Senha incorreta' }, 401, corsHeaders);
    }

    // Retornar dados do afiliado (sem senha)
...
async function handleAffiliateStats(request, env, corsHeaders) {
  try {
    const { code, password } = await request.json();
...
async function handleAffiliateSales(request, env, corsHeaders) {
  try {
    const { code, password } = await request.json();
...
async function handleUpdateCommission(request, env, corsHeaders) {
  try {
    const { password, code, commission } = await request.json();
    ...
    const affiliate = JSON.parse(affiliateData);
    affiliate.commission = parseFloat(commission);
...
async function handleToggleAffiliate(request, env, corsHeaders) {
  try {
...
async function handleMarkPaid(request, env, corsHeaders) {
  try {
...
async function handleDeleteOrder(request, env, corsHeaders) {
  try {
    const { password, orderId } = await request.json();
  ...
  try {
    ...
    return new Response(csv, {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="pedidos-tiktok-' + new Date().toISOString().split('T')[0] + '.csv"'
      }
    });
...
async function hashPassword(password) {
  // ATENÇÃO: Em produção, use uma lib de hash adequada
  // Por enquanto, apenas encode em base64 (NÃO É SEGURO!)
...
function getCheckoutHTML(affiliateCode) {
  const affiliateInput = affiliateCode ? `<input type="hidden" id="affiliateCode" value="${affiliateCode}">` : '';
  const affiliateBadge = affiliateCode ? `
    <div class="mb-6 p-4 bg-green-500/10 border border-green-500/30 rounded-xl">
      <div class="flex items-center gap-2 text-green-400">
        ...
        <span class="font-bold">Link de Afiliado Detectado: ${affiliateCode}</span>
      </div>
    </div>
  ` : '';
...
<head>
    ...
    <title>Checkout - TikTok Shop UK Masterclass</title>
        ...
        <div id="step1" class="glass rounded-2xl p-6 sm:p-8">
            ...
            <div class="text-center mb-8">
                ...
                <div class="mt-6">
                    ...
                    <span class="text-white text-5xl font-black ml-3">R$ 47</span>
                </div>
            </div>
        ...
        <div id="step2" class="glass rounded-2xl p-6 sm:p-8 hidden">
            ...
            <div class="bg-black/40 border border-white/10 rounded-xl p-5 mb-6">
                ...
                <div class="flex gap-3">
                    <input
                        ...
                        id="pixCode"
                        readonly
                        class="input-field flex-1 px-3 py-3 rounded-lg text-xs font-mono"
                    >
                    <button
                        onclick="copyPix()"
            ...
            <div class="bg-blue-500/10 border border-blue-500/30 rounded-xl p-5 mb-6">
                ...
                <ol class="space-y-2 text-gray-300 text-sm">
                    ...
                    <li class="flex gap-3">
                        ...
                        <span>Confirme o pagamento de <strong>R$ 47,00</strong></span>
                    </li>
                </ol>
            </div>
            ...
            <div class="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-5 text-center">
                ...
                <p class="text-gray-400 text-xs">
                    Após confirmar o pagamento, você receberá o acesso completo<br class="hidden sm:block">
                    por WhatsApp e Email automaticamente
                </p>
            </div>
            ...
            <div class="mt-6 pt-6 border-t border-white/10">
                ...
                <div class="glass rounded-lg p-4 space-y-2 text-sm">
                    <p class="text-gray-400">
                        ...
                        <span class="text-white font-semibold" id="confirmName"></span>
                    </p>
                    <p class="text-gray-400">
                        <span class="text-gray-500">WhatsApp:</span>
                        <span class="text-white font-semibold" id="confirmWhatsapp"></span>
                    </p>
                    <p class="text-gray-400">
                        <span class="text-gray-500">Email:</span>
                        <span class="text-white font-semibold" id="confirmEmail"></span>
                    </p>
                </div>
            </div>
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const formData = {
                ...
                affiliateCode: document.getElementById('affiliateCode')?.value || null
            };
...
const AFFILIATE_REGISTER_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
const AFFILIATE_PANEL_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
